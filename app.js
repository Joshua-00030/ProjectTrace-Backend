...
app.post('/testdata/nih', (req, res, next) => {
	pool.query('Select * from nsf_nih join topics on nsf_nih.topic_id = topics.topic_id order by id limit 20')
		.then(testData => {
...
app.post('/testdata/general', (req, res, next) => {
	const s = req.body.reduce((st, item) =>{
		if(item.label === 'index'){}
		/* funding is stored as text. needs to be num for between i believe
		else if(item.label === 'funding'){
			if(item.value && !isNaN(Number(item.value))){
				st.push(Number(item.value) / 1.1)
				st.push(Number(item.value) * 1.1)
			}
			else{
				st.push('')
...
/*
app.post('/testdata/getProject', (req, res, next) => {
	NIHPool.query("select * from all_nih join side_nih on project_title = title and title = $1 limit 1", [req.body.title], function (err, result){
